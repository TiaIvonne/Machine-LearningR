...
cruzadalogistica <- function(
    data = data, vardep = NULL,
  ...
  control <- trainControl(
    method = "repeatedcv", number = grupos, repeats = repe,
...
cruzadaavnnetbin <-
  function(
    ...
    if (any(listclass == c("")) == FALSE) {
      ...
      databis <- dummy_cols(databis, listclass, sep = ".")
    } else {
